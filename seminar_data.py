...
대상 분야: 재생에너지, ESS, 전기차충전, 전력, 수소, CCS, 배터리
매월 둘째·넷째 월요일 Claude와 대화로 업데이트 후 GitHub에 반영
"""

from datetime import datetime

DOMAINS = ["재생에너지", "ESS", "전기차충전", "전력", "수소", "CCS", "배터리"]

DASHBOARD_URL = "https://energy-seminar.vercel.app"
...
def build_message(changes: dict = None) -> str:
    now   = datetime.now()
    week  = (now.day - 1) // 7 + 1
    month = now.strftime("%Y년 %-m월")
    ...
    lines = [
        f"[에너지 세미나 업데이트 | {month} {week}주차]",
        f"업데이트: {now.strftime('%Y.%m.%d')}",
        "",
        f"전체 {total}건 | 확정 {conf} | 조율중 {plan} | 추정 {est} | 최우선 {high}",
        "",
    ]

    if changes and any(changes.values()):
        lines.append("-- 이번 업데이트 변경사항 --")
        lines.append("")

        if changes.get("added"):
            lines.append(f"[신규 추가] {len(changes['added'])}건")
            for i, s in enumerate(changes["added"], 1):
                lines.append(f"{i}. {s['title']}")
                lines.append(f"   일시: {s['date']}")
                lines.append(f"   주관: {s['org']}")
                lines.append(f"   비용: {s['cost']}")
                lines.append(f"   상태: {s['status']} / {s['priority']}")
                lines.append("")

        if changes.get("updated"):
            lines.append(f"[내용 변경] {len(changes['updated'])}건")
            for i, item in enumerate(changes["updated"], 1):
                lines.append(f"{i}. {item['title']}")
                for field, (old, new) in item["diffs"].items():
                    lines.append(f"   {field}: {old} → {new}")
                lines.append("")

        if changes.get("removed"):
            lines.append(f"[삭제] {len(changes['removed'])}건")
            for i, s in enumerate(changes["removed"], 1):
                lines.append(f"{i}. {s['title']}")
            lines.append("")
    else:
        lines.append("-- 이번 업데이트 변경사항 없음 --")
        lines.append("")
